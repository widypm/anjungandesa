...
async function getForm(id: number) {
  ...
  let query = null;
  if (id > 0) {
    query = await prisma.template.findUnique({
      where: { id },
    });
...
async function saveOrUpdateUser(data: any, userToken: any) {
  const { id, name, code } = data[0].value;
  ...
  if (id) {
    ...
    return { success: "Template updated", data: updated };
  } else {
    // ➕ Create user
    const data = {
      name,
    ...
    return { success: "Template created", data: created };
  }
}
