...
export default function Header({ dataApi }: Props) {
  ...
  return (
    <header className="backdrop-blur bg-white/80 border-b border-gray-200 fixed w-full z-40 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        >
          <img
            src="/images/logo-tpr-solusi-apps-ori.png"
        ...
        <nav className="flex items-center gap-6">
          <ul className="hidden md:flex space-x-8 font-medium text-gray-700">
            {dataApi?.navigation?.map((rw: any, index: number) => (
              <li key={index} className="relative group">
                <a
                  className="hover:text-purple-600 flex items-center relative pb-1 transition-colors duration-200"
                ...
                >
                  ...
                  <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-gradient-to-r from-sky-500 to-purple-500 transition-all duration-300 group-hover:w-full"></span>
                </a>

                {/* Dropdown Desktop */}
                {rw?.children?.length > 0 && (
                  <ul className="absolute left-0 mt-3 w-60 bg-white/95 rounded-xl shadow-lg opacity-0 scale-95 transform transition-all duration-200 group-hover:opacity-100 group-hover:scale-100 overflow-hidden">
                    {rw.children.map((child: any, idx: number) => (
                      <li key={idx}>
                        <a
                          href={child.url}
                          className="block px-5 py-3 hover:bg-gradient-to-r hover:from-sky-50 hover:to-purple-50 transition-colors duration-200"
                        >
                          {child.title}
                        </a>
                      </li>
          ...
          <div className="hidden md:flex items-center gap-2">
            <button
              ...
              className={
                ...
                "flex items-center gap-2 px-3 py-1.5 border border-gray-300 rounded-full text-sm font-medium text-gray-700 hover:border-purple-500 hover:text-purple-600 transition-all duration-200"
              }
            >
          ...
          <button
            ...
            className="md:hidden text-sky-600 text-3xl p-3 rounded-lg hover:bg-sky-50 transition"
      ...
      {/* Mobile Navigation */}
      {menuOpen && (
        <div className="md:hidden bg-white/95 border-t border-gray-200 backdrop-blur animate-slideDown">
